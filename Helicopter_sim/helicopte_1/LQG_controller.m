...
W = diag([1, 10, 0.1, 100000]); 
V = 0.001; 

% 这里的 G 为单位矩阵 eye(4)，即假定噪声直接作用于所有状态
% 使用 W 和 V 计算观测器增益 Kf
[Kf, ~, ~] = lqe(A, eye(4), C, W, V);
...
%% Testing pole position for LQI and LQE
% 计算极点
poles_lqi = eig(A_aug - B_aug * K_c);
poles_lqe = eig(A - Kf * C);
...
% 绘制 LQI 极点 (蓝色 'x')
plot(real(poles_lqi), imag(poles_lqi), 'bx', 'MarkerSize', 10, 'LineWidth', 1.5);

% 绘制 LQE 极点 (红色 'o')
plot(real(poles_lqe), imag(poles_lqe), 'ro', 'MarkerSize', 10, 'LineWidth', 1.5);
...
% 标注
legend('LQI (Regulator) Poles', 'LQE (Observer) Poles', 'Location', 'best');
xlabel('Real Axis');
ylabel('Imaginary Axis');
title('Pole Distribution: LQI vs. LQE');
axis equal; % 保持比例，防止扭曲
hold off;
